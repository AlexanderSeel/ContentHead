...
import { downloadJson } from '../../ui/commands/utils';
import type { Command, CommandContext } from '../../ui/commands/types';

// Types, utilities and command registrations are in dedicated modules
import type {
  CType, Template, CItem, CVersion, CRoute, PageTreeNodeDto,
  VariantSet, Variant, CompositionArea, ComponentRecord, ComponentInstance,
  ComponentTypeSettingRow,
  AclEntry, PrincipalGroup, VisitorGroup, InternalRole, InternalUser, FormListRow,
  OnPageTargetType, OnPageActionItem, OnPageTarget, OnPageListMode,
  OnPageListDialogState, OnPageListItemEditorState,
  AiMode, WorkspaceMode, PreviewDevice, TreeRow,
  ContentPageHeaderCommandContext, ContentPageRowCommandContext, ContentPageTreeCommandContext
} from './contentPageTypes';
import { CONTENT_PAGES_SHOW_ARCHIVED_KEY, PAGE_ACL_ACTIONS } from './contentPageTypes';
import {
  parseJson, parseGraphqlErrorCode,
  parseComponentInstances, parseLegacyComponentMap, componentMapFromInstances,
  compositionFromInstances, serializeComponentInstances,
  buildContentEditorUrl, getItemStatus,
  fieldPathToKey, parseComponentFieldPath, parseComponentPath,
  setNestedValue, getNestedValue, looksLikeContentLinkValue,
  sanitizeForAttribute, parseTemplateIdFromMetadata
} from './contentPageUtils';
import { contentPageHeaderOverflowCommands, contentPageRowOverflowCommands, contentPageTreeContextCommands } from './contentPageCommands';
      ...
      <DialogPanel header="Add Component" visible={showAddComponent} onHide={() => setShowAddComponent(false)} className="w-11 md:w-8 lg:w-6 xl:w-4">
        <div className="form-row">
          ...
          <Select
            ...
            onChange={(next) => { if (next != null) setNewComponentType(next); }}
          />
        </div>
        <div className="form-row">
          <label>Area</label>
          <Select
            ...
            onChange={(next) => { if (next != null) setNewComponentArea(next); }}
          />
        </div>
