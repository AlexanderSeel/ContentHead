...
export type WorkflowNodeRegistryEntry = {
  type: string;
  label: string;
  icon: string;
...
export const nodeRegistry: WorkflowNodeRegistryEntry[] = [
  {
    type: 'FetchContent',
    label: 'Fetch Content',
    icon: 'pi pi-search',
...
export function getNodeRegistryEntry(type: string): WorkflowNodeRegistryEntry | null {
  return nodeRegistry.find((entry) => entry.type === type) ?? null;
}

export function validateNodeConfig(type: string, value: Record<string, unknown>): string[] {
