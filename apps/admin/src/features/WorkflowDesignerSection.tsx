...
import { WorkspacePaneLayout } from '../ui/molecules';
...
export function WorkflowDesignerSection({
  ...
  return (
    <section className="splitFill workflow-designer-section">
      <WorkspacePaneLayout
        workspaceId="workflows-designer"
        className="workflow-designer-split"
        left={{
          id: 'definitions',
          label: 'Definitions',
          defaultSize: 25,
          ...
          headerActions: (
            <div className="inline-actions">
              ...
              <Button
                ...
                severity="success"
                onClick={() => startRun().catch((error: unknown) => onStatus(formatErrorMessage(error)))}
              />
              <Button label="Remove Node" severity="danger" text disabled={!selectedNodeId} onClick={removeSelectedNode} />
            </div>
          ),
          content: (
            <ReactFlow
